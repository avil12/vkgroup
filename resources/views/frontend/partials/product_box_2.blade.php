...
@endphp
<div class="aiz-card-box px-3 h-auto bg-white py-3 hov-scale-img">
    <div class="position-relative h-300px  img-fit overflow-hidden">
        @php
    ...
    <div class="p-2 p-md-3 text-left">
        <!-- Product name -->
        <h3 class="fw-100 fs-14 text-truncate-2 lh-1-4 mb-2  text-center">
            <a href="{{ $product_url }}" class="d-block  hov-text-primary"  style="font-family: 'RobotoMedium',Arial;color: #454545" title="{{  $product->getTranslation('name')  }}">{{  $product->getTranslation('name')  }}</a>
        </h3>
        <div class="fs-12 d-flex justify-content-center mt-1">
            @if($product->auction_product == 0)
                <!-- Previous price -->
                @if(home_base_price($product) != home_discounted_base_price($product))
                    <!-- <div class="disc-amount has-transition">
                        <del class="fw-300 text-secondary mr-1">{{ home_base_price($product) }}</del>
                    </div> -->
                @endif
                <!-- price -->
               <!--  <div class="">
                    <span class="fw-500" style="color: #848484;font-family: Manrope,sans-serif">Starting {{ home_discounted_base_price($product) }}</span>
                </div> -->
            @endif
            @if($product->auction_product == 1)
                <!-- Bid Amount -->
                <div class="">
                    <span class="fw-500 text-primary">{{ single_price($product->starting_bid) }}</span>
                </div>
            @endif
        </div>
    </div>
